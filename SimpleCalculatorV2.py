# Created with tkinter and tkinter bootstrap
import math
import tkinter as tk
import ttkbootstrap as ttk

# window init
app = ttk.Window(themename="journal")   # darkly, flatly, lumen, minty
...
visual_display = tk.Entry(master=app, textvariable=input_display, font="Arial 22", bd=6, justify="right")
visual_display.grid(row=0, column=0, columnspan=4, ipadx=23, ipady=18, padx=2, pady=7)
...
for i, row in enumerate(button):
    for j, btn_text in enumerate(row):
        btn = tk.Button(master=app, text=btn_text, font="Arial 20 bold",
                        command=lambda text=btn_text: click(text))
        btn.grid(row=i + 1, column=j, ipadx=8, ipady=6, columnspan=1, padx=4, pady=4, sticky="nsew")
